...
├── tests/                        # check.mjs (Tier-1) + eval-assert.mjs (Tier-2) + fixtures/
├── examples/eval-fixture/        # planted-defect fixture + answer key for the Tier-2 behavioral eval
├── .claude/
...
This project follows [Semantic Versioning](https://semver.org): `MAJOR.MINOR.PATCH`. Bump PATCH for fixes, MINOR for new features (a new specialization or format), MAJOR for breaking changes to how skills or config work. Releases are tagged `vX.Y.Z` and recorded in [`CHANGELOG.md`](CHANGELOG.md). Current version: **0.11.0**.

## Contributing

Issues and pull requests welcome. Good first contributions: new `specializations/*.md` modules, additional file-format handling, and glossary tooling. Please add a changelog entry under `[Unreleased]` with your change, and run `npm install && npm test` before submitting — a deterministic checker (`tests/check.mjs`) guards version consistency, the changelog, config-schema health, cross-references, and frontmatter.
